...
class combined_USTC_mal(Dataset):
...
class MIXED44(Dataset):

    def __init__(self, root, mode='train', labeled_num=5, labeled_ratio=0.5, rand_number=0, transform=None, target_transform=None,
                 download=False, unlabeled_idxs=None):
        self.transform = transform
        self.target_transform = target_transform

        if mode == 'train':
            loaded_data = np.load(r'data/dataset/mixed_44_train.npz')
        elif mode == 'val':
            loaded_data = np.load(r'data/dataset/mixed_44_val.npz')
        elif mode == 'test':
            loaded_data = np.load(r'data/dataset/mixed_44_test.npz')
        else:
...
def get_dataset(dataset, train=False, select_classes=None, target_transform=None, mode=None):
    if dataset == 'mal':
        ...
        # mode 参数指定 train/val/test，优先于旧版 train 参数
        if mode is None:
            mode = 'train' if train else 'test'
        if mode == 'train':
            dataset_raw = MIXED44(root=data_dir, mode='train', transform=transform_train, download=False)
        elif mode == 'val':
            dataset_raw = MIXED44(root=data_dir, mode='val', transform=transform_eval, download=False)
        elif mode == 'test':
            dataset_raw = MIXED44(root=data_dir, mode='test', transform=transform_eval, download=False)
        else:
            raise ValueError(f"Unsupported mode: {mode}")
        train_set = dataset_raw
        test_set = dataset_raw  # 统一用 dataset_raw，上层调用时用 mode 区分
