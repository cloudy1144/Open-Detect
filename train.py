...
from model import OpenDetectNet, train_model, validate_model, test_model
...
if __name__ == '__main__':
    ...
    if args.dset == 'mixed_44':
        train_set = get_dataset(known_dataset, select_classes=known_classes, target_transform='reindex', mode='train')
        val_set = get_dataset(known_dataset, select_classes=known_classes, target_transform='reindex', mode='val')
        test_set = get_dataset(known_dataset, select_classes=known_classes, target_transform='reindex', mode='test')
        open_set = None
    else:
        ...
        open_set = get_dataset(unknown_dataset, False, unknown_classes, 'open') if unknown_dataset is not None and len(unknown_classes) > 0 else None

    print(len(val_set), len(open_set) if open_set is not None else 0)
    ...
    if args.dset == 'mixed_44':
        test_loader = DataLoader(test_set, batch_size=64, shuffle=False, num_workers=4, drop_last = False)
    ...
    # 对 mixed_44 进行最终测试集评估
    if args.dset == 'mixed_44':
        print('\n' + '=' * 70)
        print('Final Test Set Evaluation')
        print('=' * 70)
        test_model(open_detect_net, args, test_loader, args.epoch)