...
def train_model(model, args, train_loader, epoch, optimizer):
    ...
    all_preds = []
    all_labels = []
    pbar = tqdm(train_loader, desc='Train Epoch {}'.format(epoch + 1), ncols=100)
    for i, (image, label) in enumerate(pbar):
        ...
        all_labels.extend(label.cpu().data.numpy())
        # Update progress bar with real-time metrics
        current_acc = train_corrects.item() / ((i + 1) * train_loader.batch_size)
        pbar.set_postfix({'loss': '{:.3f}'.format(total_loss.item()),
                          'acc': '{:.3f}'.format(current_acc)})
    ...
    print('Train corrects: {} Train samples: {} Train accuracy: {:.4f} Train F1: {:.4f}'.format(
        train_corrects, len(train_loader.dataset), train_acc, train_f1))
...
def validate_model(model, args, val_loader, epoch):
    ...
    all_preds = []
    all_labels = []
    pbar = tqdm(val_loader, desc='Val   Epoch {}'.format(epoch + 1), ncols=100)
    for image, label in pbar:
        with torch.no_grad():
            ...
            all_preds.extend(preds.cpu().data.numpy())
            all_labels.extend(label.cpu().data.numpy())
            current_acc = val_corrects.item() / (len(all_labels))
            pbar.set_postfix({'loss': '{:.3f}'.format(total_loss.item()),
                              'acc': '{:.3f}'.format(current_acc)})
    val_acc = val_corrects / len(val_loader.dataset)
    val_f1 = f1_score(all_labels, all_preds, average='weighted')
    val_loss = {k: val_running_loss.get(k, 0) / len(val_loader) for k in val_running_loss.keys()}
    print('Val corrects: {} Val samples: {} Val accuracy: {:.4f} Val F1: {:.4f}'.format(
        val_corrects, len(val_loader.dataset), val_acc, val_f1))
...
def test_model(model, args, test_loader, epoch):
    # Test loop for final evaluation
    model.eval()
    test_corrects = 0.0
    test_running_loss = {'total': 0.0, 'rec': 0.0, 'kld': 0.0, 'ent': 0.0, 'dis': 0.0}
    all_preds = []
    all_labels = []
    pbar = tqdm(test_loader, desc='Test  Final', ncols=100)
    for image, label in pbar:
        with torch.no_grad():
            image, label = image.cuda(), label.cuda()
            latent_z, x_recon, preds, loss = model.loss(image, label)
            ...
            loss['total'] = total_loss
            for k in loss.keys():
                test_running_loss[k] = loss.get(k, 0).item() + test_running_loss.get(k, 0)
            test_corrects += torch.sum(preds == label.data)
            all_preds.extend(preds.cpu().data.numpy())
            all_labels.extend(label.cpu().data.numpy())
            current_acc = test_corrects.item() / (len(all_labels))
            pbar.set_postfix({'loss': '{:.3f}'.format(total_loss.item()),
                              'acc': '{:.3f}'.format(current_acc)})
    test_acc = test_corrects / len(test_loader.dataset)
    test_f1 = f1_score(all_labels, all_preds, average='weighted')
    test_loss = {k: test_running_loss.get(k, 0) / len(test_loader) for k in test_running_loss.keys()}
    print('Test corrects: {} Test samples: {} Test accuracy: {:.4f} Test F1: {:.4f}'.format(
        test_corrects, len(test_loader.dataset), test_acc, test_f1))
    print('Test loss: {:.3f}= {}*[rec({:.3f}) + kld({:.3f}) + ent({:.3f})] + (1-{})*dis({:.3f})'.format(
        test_loss['total'], args.lamda, test_loss['rec'], test_loss['kld'],
        test_loss['ent'], args.lamda, test_loss['dis']))
    print('*' * 70)
    return test_acc
